<template>
    <div class="dashboard row border-bottom pb-4">
        ...
        <div class="dashboard__item col-12 d-flex justify-content-center">
            ...
            >
                <div class="dashboard-infor">
                    <p class="item__name mb-1">Tài khoản giám sát</p>
                    <h3 class="item__quantity fw-bold">{{ ministry }}</h3>
                </div>
                <img
...
export default {
    ...
    async created() {
        ...
        for (let i = 0; i < accounts.length; i++) {
            if (accounts[i].rule == "MINISTRY") this.ministry++;
            else if (accounts[i].rule == "TEACHER") this.teacher++;
            else if (accounts[i].rule == "STUDENT") this.student++;
        }

        this.total = this.ministry + this.teacher + this.student;
    },
};
</script>
